...
app.config['SQLALCHEMY_DATABASE_URI']= 'mysql+pymysql://root:FranciscoPelaez_1995-1@localhost:3306/BancoPichincha'
...
#Creamos un esquema
class BancoPchinchaSchema(ma.Schema):
    class Meta:
        fields= ('cedula','nombre','cuenta','saldo','entidadB')

#Una sola respuesta
cliente_schema= BancoPchinchaSchema()
client_schema= BancoPchinchaSchema()
#Varias Respuestas
clientes_schema= BancoPchinchaSchema(many=True)

#Metodo GET Clientes del Banco
@app.route('/list',methods=['GET'])
def get_clientes():
    all_clientes = BancoP.query.all()
...
def get_clientes_id(id):
    un_cliente= BancoP.query.get(id)
...
def addCuenta():
    ...
    nuevo= BancoP(cedula, nombre, cuenta, saldo, entidadB)
...
def retiro(cuenta, monto): 
    ...
    un_cliente= BancoP.query.filter_by(cuenta=cuenta).first()
...
def tranferencia(cuenta, banco1, monto, cuenta2, banco2): 
    ...
    cliente1= BancoP.query.filter_by(cuenta=cuenta).first()
    #Convertirmos el valor string en decimal
    ...
    if banco2 == 'pacifico':
        ...
        bancoPichincha= requests.put('http://127.0.0.1:9567/deposito/'+cuenta2+'/'+cantidadT)
    ...
    if banco2=='pichincha':
        print('Desposito en el Banco Pichincha ', 'cantidad: ', cantidadE)
        cliente2= BancoP.query.filter_by(cuenta2=cuenta2).first()
        #Realizamos la operacion
